...
void extendStack(Typer * env, i32 cap, Term ** items);
void addToStack(Typer * env, Term * value);
b32 equal(ConstructorMap * map, Term * lhs, Term * rhs);
Term * rebase(MemoryArena * arena, Term * in0, i32 delta, i32 offset);
Term * rebase(MemoryArena * arena, Term * in0, i32 delta);
Term * evaluate(MemoryArena * arena, Stack * stack, ConstructorMap * map, Term * in0, i32 offset);
Term * evaluateWithArgs(MemoryArena * arena, ConstructorMap * map, i32 arg_count, Term ** args, Term * in0);
CompareTerms compareTerms(MemoryArena * arena, ConstructorMap * map, Term * lhs0, Term * rhs0);
Trinary equalTrinary(ConstructorMap * map, Term * lhs0, Term * rhs0);
Term * normalize(MemoryArena * arena, ConstructorMap * map, Term * in0);
...
Term * newRewrite(MemoryArena * arena, Typer * env, Term * eq_proof, Term * body, TreePath * path, b32 right_to_left);
