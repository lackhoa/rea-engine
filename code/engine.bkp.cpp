  ...
  else if (lhs0->cat == rhs0->cat)
  {
    switch (lhs0->cat)
    {
      case VC_StackValue:
      {
        ...
        if (lhs->relative_depth || rhs->relative_depth) todoIncomplete;
      ...
      case VC_ArrowV:
      {
        ...
        if (rhs->param_count == param_count)
        {
          ...
          {
            if (!equalB32(lhs->param_types[id], rhs->param_types[id]))
            {
              type_mismatch = true;
              break;
            }
...
inline b32
hasFreeVars(Value *in0, i32 stack_offset)
{
  switch (in0->cat)
  {
    case VC_StackValue:
    {
      StackValue *in = castValue(in0, StackValue);
      if (in->relative_depth && in->stack_depth == stack_offset)
        return true;
    } break;
...
internal Value *
processArrow(MemoryArena *arena, i32 depth, Value *in0)
{// remove all deep variables, as well as heap values
  Value *out0 = 0;
  switch (in0->cat)
  {
    case VC_StackValue:
    {
      ...
      if (in->stack_depth <= depth)
      {
        ...
        out->relative_depth = true;
        out->stack_depth    = depth - in->stack_depth;
        out0 = &out->v;
      }
    } break;
    ...
    {
      HeapValue *in = castValue(in0, HeapValue);
      out0 = processArrow(arena, depth, &in->accessor.v);
    } break;
    ...
    case VC_AccessorV:
    {
      ...
      out->record = processArrow(arena, depth, in->record);
    } break;
    ...
    case VC_CompositeV:
    {
      CompositeV *in  = castValue(in0, CompositeV);
      CompositeV *out = copyStruct(arena, in);
      out->op = processArrow(arena, depth, in->op);
      allocateArray(arena, in->arg_count, out->args);
      for (i32 id=0; id < in->arg_count; id++)
      {
        out->args[id] = processArrow(arena, depth, in->args[id]);
      }
    } break;
    ...
    case VC_ArrowV:
    {
      ...
      for (i32 id=0; id < in->param_count; id++)
        out->param_types[id] = processArrow(arena, depth+1, in->param_types[id]);
      out->output_type = processArrow(arena, depth+1, in->output_type);
    } break;
...
internal Value *
evaluateArrow(MemoryArena *arena, Value **args, Value *in0, i32 offset)
{
  Value *out0 = 0;
  switch (in0->cat)
  {
    ...
    {
      StackValue *in = castValue(in0, StackValue);
      if (in->relative_depth && in->stack_depth == offset)
        out0 = args[in->id];
      else out0 = in0;
    } break;
    ...
    case VC_CompositeV:
    {
      CompositeV *in  = castValue(in0, CompositeV);
      CompositeV *out = copyStruct(arena, in);
      out->op = evaluateArrow(arena, args, in->op, offset);
      allocateArray(arena, in->arg_count, out->args);
      for (i32 id=0; id < in->arg_count; id++)
        out->args[id] = evaluateArrow(arena, args, in->args[id], offset);
    } break;
    ...
    case VC_ArrowV:
    {
      ...
      {
        out->param_types[id] = evaluateArrow(arena, args, out->param_types[id], offset+1);
      }
      out->output_type = evaluateArrow(arena, args, out->output_type, offset+1);
    } break;
...
#define DEBUG_EVALUATE_DEPTH 2
  ...
  switch (in0->cat)
  {
    ...
    case AC_Composite:
    {
      ...
      Value *output_type = evaluateArrow(arena, args, signature->output_type, 0);
      CompositeV *out = newValue(arena, CompositeV, output_type);
      out->arg_count = in->arg_count;
      out->op        = op;
      out->args      = args;
      out0 = &out->v;
    ...
    case AC_Arrow:
    {
      ...
      for (int id=0; id < out->param_count; id++)
      {
        Value *param_type = evaluate(arena, env, in->param_types[id]);
        introduceOnStack(env, out->param_names+id, param_type);
        out->param_types[id] = processArrow(arena, env->stack->depth, param_type);
      }
      Value *output_type = evaluate(arena, env, in->output_type);
      out->output_type = processArrow(arena, env->stack->depth, output_type);
      unwindStack(env);
...
introduceOnHeap(Environment *env, Value *parent, Constructor *ctor)
{
  ...
  if (ArrowV *ctor_sig = castValue(ctor->type, ArrowV))
  {
    ...
    Value *record_type = evaluateArrow(temp_arena, env->stack->items, ctor_sig->output_type, 0);
    ...
    Environment sig_env = *env;
    {
      ...
      for (s32 field_id=0; field_id < param_count; field_id++)
      {
        ...
        Value *member_type = evaluateArrow(temp_arena, env->stack->items, ctor_sig->param_types[field_id], 0);
...
introduceOnStack(Environment *env, Token *name, Value *type)
{
  Value *intro;

  StackValue *ref = newValue(temp_arena, StackValue, type);
  ref->name        = *name;
...
introduceOnStack(Environment *env, Token *name, Ast *type)
{
  ...
  introduceOnStack(env, name, typev);
}
    ...
    if (GlobalBinding *slot = lookupGlobalNameSlot(ident->token, false))
    {
      if (isGlobalValue(expected_type))
      {
        ...
        for (int slot_id=0; slot_id < slot->count; slot_id++)
        {
          ...
          {
            if (!equalB32(signature->output_type, expected_type))
              ...
              for (int attempt=0; attempt < retry_count; attempt++)
              {
                ...
                if (noError())
                {
                  ...
                  if (ArrowV *arrowv = castValue(hint->type, ArrowV))
                  {
                    ...
                    if (unify(values, arrowv->output_type, &eq->v))
                    {
          ...
          if (ArrowV *signature = castValue(op.value->type, ArrowV))
          {
            ...
            if (noError())
            {
                   ...
                   arg_id++)
              {
                ...
                Value *expected_arg_type = evaluateArrow(temp_arena, args, param_type0, 0);
                ...
                else
                {
                  if (Expression arg = buildExpression(arena, env, in->args[arg_id], expected_arg_type))
                  {
                    ...
                    if (expected_arg_type == holev)
                    {
                      ...
                      assert(param_type->stack_depth == 0);
