...
internal Value *
fillHole(MemoryArena *arena, Typer *env, Value *goal)
{
  Value *out = 0;
...
inline Value *
newEquality(MemoryArena *arena, Typer *env, Value *lhs, Value *rhs)
{
  ...
  else
  {
    switch (in0->cat)
    {
      ...
      case Term_Composite:
      {
        ...
        out0 = evaluateWithArgs(arena, in->arg_count, in->args, op_type->output_type);
      } break;
...
forward_declare inline void
extendStack(Typer *env, i32 cap, Value **items)
{
...
forward_declare inline void
addToStack(Typer *env, Value *value)
{
...
overwriteTerm(Typer *env, Term *in)
{
  ...
  if (true || isOverwritable(in))
  {
    for (OverwriteRules *rule = env->overwrite;
         ...
         rule = rule->next)
    {
      if (equal(in, rule->lhs))
      {
        if (Composite *record = castTerm(rule->rhs, Composite))
        {
          Composite *out = copyStruct(temp_arena, record);
  ...
  if (!isGlobalConstant(in0))
  {
    switch (in0->cat)
    {
      ...
      case Term_Accessor:
      {
        Accessor *in  = castTerm(in0, Accessor);
        Accessor *out = copyStruct(arena, in);
        out->record = evaluate(arena, stack, in->record, offset);
        out0 = &out->t;
      } break;
      ...
      case Term_Fork:
      {
        ...
        {// note: we fail if the fork is undetermined
          case Term_Constructor:
          {
            ctor = castTerm(subject, Constructor);
          } break;
...
forward_declare internal Term *
normalize(MemoryArena *arena, Typer *env, Value *in0) 
{
  Value *out0 = {};
...
inline Term *
makeFakeRecord(MemoryArena *arena, Typer *env, Value *parent, Constructor *ctor)
{
  Value *out = 0;
  if (Arrow *ctor_sig = castTerm(ctor->type, Arrow))
  {
    ...
    for (i32 field_id=0; field_id < param_count; field_id++)
    {
      ...
      Value *member_type = evaluateWithArgs(arena, param_count, record->args, ctor_sig->param_types[field_id]);
      ...
      {// recursive case
        Value *intro = makeFakeRecord(arena, env, &accessor->t, field_ctor);
...
#if 1
internal b32
addOverwriteRule(Typer *env, Term *lhs0, Term *rhs0)
{
  b32 added = false;
...
internal b32
unify(Typer *env, Value **args, Term **types, Term *lhs0, Value *rhs0)
{
  b32 success = false;
  b32 debug = true;
...
buildTerm(MemoryArena *arena, Typer *env, Ast *in0, Value *goal)
{
      ...
      else
      {
        ...
        if (GlobalBinding *globals = lookupGlobalName(name))
        {
          for (s32 value_id = 0; value_id < globals->count; value_id++)
          {
            ...
            if (matchType(slot_value->type, goal))
            {
              if (value)
              {// ambiguous
        ...
        if (Arrow *signature = castTerm(getType(env, op), Arrow))
        {
          ...
          if (noError())
          {
                 ...
                 arg_id++)
            {
              ...
              Term *expected_arg_type = evaluateWithArgs(temp_arena, signature->param_count, args, param_type0);
    ...
    if (Union *uni = castTerm(getType(env, subject.term), Union))
    {
      if (uni->ctor_count == case_count)
      {
             ...
             input_case_id++)
        {
          ...
          if (ctor)
          {
            Term *record = makeFakeRecord(temp_arena, env, subject.term, ctor);
            addOverwriteRule(env, subject.term, record);
