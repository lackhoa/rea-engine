...
inline void
addConstructorRecord(Environment *env, Value *value, Constructor *ctor)
{
  ConstructorRecord *record = pushStruct(temp_arena, ConstructorRecord);
  record->value   = value;
  record->ctor    = ctor;
  record->next    = env->ctor_record;
  env->ctor_record = record;
}

inline void
removeLastConstructorRecord(Environment *env)
{
  env->ctor_record = env->ctor_record->next;
}

inline Constructor *
getConstructor(Environment *env, Value *value)
{
  Constructor *ctor = 0;
  switch (value->cat)
  {
    case VC_Constructor:
    {
      ctor = castValue(value, Constructor);
    } break;

    case VC_CompositeV:
    {
      CompositeV *record = castValue(value, CompositeV);
      ctor = castValue(record->op, Constructor);
    } break;
  ...
  if (!ctor)
  {
    for (ConstructorRecord *record=env->ctor_record; record; record=record->next)
    {
      if (equalB32(value, record->value))
        return ctor = record->ctor;
    }
  }
  return ctor;
}
...
evaluateFork(MemoryArena *arena, Environment *env, Fork *fork)
{
  ...
  Constructor *ctor = getConstructor(env, subject);
  if (ctor)
  {
    addStackFrame(env);
...
#define DEBUG_EVALUATE_DEPTH 2
  ...
  switch (in0->cat)
  {
    ...
    {
      Accessor *in = castAst(in0, Accessor);
      Value *record = evaluate(arena, env, in->record);
      Constructor *ctor = getConstructor(env, record);
      ArrowV *ctor_type = castValue(ctor, ArrowV);
      addStackFrame(env);
      for (i32 id=0; id < in->field_id; id++)
      {
        addStackValue(env, evaluate(arena, env, ctor_type->param_types[in->field_id]));
      }
      AccessorV *out = newValue();
      evaluate(arena, env, ctor_type->param_types[in->field_id])
      out0 = record->args[];
      unwindStack(env);
    } break;
      ...
      if (uni->ctor_count == case_count)
      {
             ...
             input_case_id++)
        {
          ...
          if (GlobalBinding *lookup = lookupGlobalName(ctor_token))
          {
            ...
            if (ctor)
            {
              addConstructorRecord(env, subjectv, ctor);
              ...
              removeLastConstructorRecord(env);
            }
            else
            {
  ...
  switch (in0->cat) check_switch(expression)
  {
    ...
    case AC_Accessor:
    {
      ...
      if (Expression build_record = buildExpression(arena, env, in->record, holev))
      {
        ...
        in->record = record;
        Constructor *ctor = getConstructor(env, recordv0);
        b32 accessor_valid = false;
        if (ctor)
        {
          if (ArrowV *ctor_type = castValue(ctor->type, ArrowV))
          {
            accessor_valid = true;
            s32 param_count = ctor_type->param_count;
            b32 valid_param_name = false;
            for (s32 param_id=0; param_id < param_count; param_id++)
            {// figure out the param id
              if (equal(in->field_name, ctor_type->param_names[param_id]))
              {
            ...
            else
            {
              ...
              attach("expected a member of constructor", &ctor->v);
              attach("in type", ctor_type->output_type);
            }
          }
        }
        if (!accessor_valid)
